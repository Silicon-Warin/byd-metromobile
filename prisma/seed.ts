...
async function main() {
  ...
  for (const variant of variants) {
    ...
    await prisma.lightingFeatures.upsert({
      ...
      create: {
        ...
        rgbDynamicMoodLights: true, // ระบบแสงไฟในห้องโดยสารปรับตามอัจฉริยะ (RGB dynamic mood lights)
        frontReadingLights: true, // ไฟส่องแบบที่สำหรับผู้โดยสารคอนหน้าแบบ LED
        rearReadingLights: true, // ไฟส่องแบบที่สำหรับผู้โดยสารคอนหลังแบบ LED
        doorSillScuffPlates: true, // ไฟส่องสว่างในพื้นที่เก็บสัมภาระ
        variantId: variant.id
      }
    });
  ...
  console.log('All features have been added to variants!');
}
