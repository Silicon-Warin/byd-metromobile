...
export default function ContactFAB() {
	...
	// React 19 ref callback with cleanup for outside click detection
	const setupOutsideClickListener = (element: HTMLDivElement | null) => {
		if (!element) return;

		const handleClickOutside = (event: MouseEvent) => {
			if (!element.contains(event.target as Node) && isOpen && !isAnimating) {
				handleClose();
			}
		};
